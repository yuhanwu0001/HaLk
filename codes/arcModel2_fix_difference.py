...
class ArcNegation(nn.Module):
    ...
    def forward(self, center_embedding, arclength_embedding):
        indicator_lt_pi = center_embedding < pi
        indicator_geq_pi = center_embedding >= pi

        center_embedding[indicator_lt_pi] = center_embedding[indicator_lt_pi] + pi
        center_embedding[indicator_geq_pi] = center_embedding[indicator_geq_pi] - pi
...
class HaLk(nn.Module):
    ...
    def HaLkArc(self, is_train, head, relation, tail, mode, offset, rel_len, qtype, shift_of_node, head_one_hot,
        ...
        def distance_outside_inside(entity_embedding, query_center_embedding, query_arclength_embedding, shift_embedding = None):
            ...
            distance2center = torch.abs(2 * torch.sin((entity_embedding - query_center_embedding) / 2))
            ...
            distance_out = torch.min(torch.abs(2 * torch.sin(delta1 / 2)), torch.abs(2 * torch.sin(delta2 / 2)))
