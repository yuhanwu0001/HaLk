...
from arcModel1_fix_arclength_init_half import HaLk
...
def parse_args(args=None):
    ...
    parser.add_argument('--model', default='HaLkArc', type=str)
...
def save_model(model, optimizer, save_variable_list, args, before_finetune=False):
    ...
    relation_center_embedding = model.relation_center_embedding.detach().cpu().numpy()
    np.save(
        os.path.join(args.save_path, 'relation_center_embedding' if not before_finetune else 'relation_center_embedding_before'),
        relation_center_embedding
    )

    relation_arclength_embedding = model.relation_arclength_embedding.detach().cpu().numpy()
    np.save(
        os.path.join(args.save_path, 'relation_arclength_embedding' if not before_finetune else 'relation_arclength_embedding_before'),
        relation_arclength_embedding
    )
...
def main(args):
    ...
    if args.negation_structures:
        args.task = '1c.2c.3c.2i.3i.2d.3d.2in.3in.cin.cni'
    else:
        args.task = '1c.2c.3c.2i.3i.ic.ci.2u.uc.2d.3d.dc'
